...
# 'selectExpr' allows SQL-like expressions (now don't need 'col')
...
# 'withColumn' requires 'col' in 2nd argument
# 'isin' Return Boolean 'true' or 'false'
...
# Fix this using: display(eventsDF.distinct())

# Now if you need to consider only a subset of the columns when dropping duplicates, 
# then you first have to make a column selection before calling distinct() 

distinctUsersDF = eventsDF.select("user_id").distinct()
...
# 'dropDuplicates' requires [] brackets
...
# Sort by both asc() and desc()
# increaseSessionsDF = eventsDF.orderBy(col("user_first_touch_timestamp").asc(), col("event_timestamp").desc())
