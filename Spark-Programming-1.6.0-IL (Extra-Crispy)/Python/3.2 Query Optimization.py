...
ppDF = (spark.read.jdbc(
    ...
    lowerBound=1,                 # Min columnName value to decide partition stride (range of values to be fetched). Not Filter
    upperBound=1000000,           # Max columnName value to decide partition stride (range of values to be fetched). Not Filter
...
# MAGIC %sql
# MAGIC -- UNDERSTANDING 'STRIDE'
# MAGIC 
# MAGIC -- Stride:       upperBound/numPartitions - lowerBound/numPartitions 
# MAGIC -- For example:  lowerBound    = 0
# MAGIC --               upperBound    = 500
# MAGIC --               numPartitions = 5
# MAGIC -- Stide:        500/5 - 0/5 = 100
# MAGIC -- Stride specifies how many rows of a given range of partition column values can be kept within a single partition.
# MAGIC 
# MAGIC -- SELECT * FROM tableX WHERE partitionColumn < 100 or partitionColumn IS NULL
# MAGIC -- SELECT * FROM tableX WHERE partitionColumn BETWEEN 100 and 200
# MAGIC -- SELECT * FROM tableX WHERE partitionColumn BETWEEN 200 and 300
# MAGIC -- SELECT * FROM tableX WHERE partitionColumn BETWEEN 300 and 400
# MAGIC -- SELECT * FROM tableX WHERE partitionColumn > 500
