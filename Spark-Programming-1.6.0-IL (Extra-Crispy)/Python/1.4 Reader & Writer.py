...
# Spawns 2 Jobs (One from 'inferSchema')
...
# Bonus:
# Type: spark.read. Put cursor at end and hit 'Tab' key to see arguments
...
# No Jobs spawned this time since we defined the Schema manually
...
# MAGIC %sql
# MAGIC CREATE TABLE t1 
# MAGIC USING parquet
# MAGIC LOCATION 'dbfs:/mnt/train/dept/';

# COMMAND ----------

# No Jobs spawned this way either
...
# JSON can have nested columns
# Column names gets put in Alphabetic order
...
# MAGIC 
# MAGIC // Configure this app to connect to a PostgreSQL database
# MAGIC Class.forName("org.postgresql.Driver")
# MAGIC 
# MAGIC val connectionProperties = new java.util.Properties()
# MAGIC connectionProperties.put("user", "readonly")
# MAGIC connectionProperties.put("password", "readonly")
# MAGIC 
# MAGIC val tableName = "training.people_1m"
# MAGIC val jdbcUrl = "jdbc:postgresql://server1.databricks.training:5432/training"

# COMMAND ----------

# MAGIC %scala
# MAGIC sc.setJobDescription("Step B: No Predicate")
# MAGIC 
# MAGIC spark.read
# MAGIC   .jdbc(url=jdbcUrl, table=tableName, properties=connectionProperties) // Open a JDBC connect
# MAGIC   .write.format("noop").mode("overwrite").save()                       // Execute a noop write to test

# COMMAND ----------

# MAGIC %scala
# MAGIC sc.setJobDescription("Step C: With Predicate")
# MAGIC 
# MAGIC spark.read
# MAGIC   .jdbc(url=jdbcUrl, table=tableName, properties=connectionProperties) // Open a JDBC connect
# MAGIC   .filter($"id" === 343517)                                            // One and only one record
# MAGIC   .write.format("noop").mode("overwrite").save() 

# COMMAND ----------

# No Job spawned here due to pre-defined Schema
...
# Bonus: Altertnative coding:   usersDF.write.format("parquet").save('dbfs:/user/mark.ott@databricks.com/spark_programming/1_4_reader_writer/p/users.parquet', mode ="overwrite")
...
# To find out File format of SQL table you created, run below command
# %sql
# DESCRIBE table extended events_p

# COMMAND ----------

usersDF.rdd.getNumPartitions()
