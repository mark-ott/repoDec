...
# Alternative ways of coding without using a UDF
from pyspark.sql.functions import *
df1 = spark.createDataFrame([('2015-04-08',)], ['dt'])
df1.select(dayofweek('dt').alias('day')).collect()

# COMMAND ----------

# Using Approx Count Distinct
df = (spark.read.parquet(eventsPath)
  .withColumn("ts", (col("event_timestamp") / 1e6).cast("timestamp"))
  .withColumn("date", to_date("ts"))
  .groupBy("date").agg(approx_count_distinct("user_id").alias("active_users"))
...
# Using Count Distinct with 'dayofweek'
df = (spark.read.parquet(eventsPath)
  .withColumn("ts", (col("event_timestamp") / 1e6).cast("timestamp"))
  .withColumn("date", to_date("ts"))
  .groupBy("date").agg(countDistinct("user_id").alias("active_users"))
  .withColumn("day", dayofweek("date"))
  .groupBy("day").agg(avg(col("active_users")).alias("avg_users"))
  .orderBy("day")
     )
...
df7 = spark.createDataFrame([('2015-04-08',)], ['dt'])
df7.select(dayofweek('dt').alias('day')).collect()

#from pyspark.sql.functions import *
#df.select(dayofweek('day').alias('day')).collect()

# COMMAND ----------

# 'get' returns value for a specified key if the key is in a Dictionary
