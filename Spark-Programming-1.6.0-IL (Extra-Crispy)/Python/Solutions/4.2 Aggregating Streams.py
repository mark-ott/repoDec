...
# Bonus: Show Streaming DataFrame
display(eventsDF)
...
# Bonus: Alternative ANSWER using 'hour' column instead of the 'window.start' method
spark.conf.set("spark.sql.shuffle.partitions", sc.defaultParallelism)

from pyspark.sql.functions import approx_count_distinct, hour, window

trafficDF = (eventsDF.groupBy("traffic_source", "hour", window(col("createdAt"), "1 hour")).agg(
    approx_count_distinct("user_id").alias("active_users"))
  .select(col("traffic_source"), col("active_users"), (col("hour")))
  .sort("hour")
)
...
# Understanding 'window.start' and 'window.end'

# Create Dataset
from pyspark.sql.functions import *
df = spark.createDataFrame([("2021-04-20 09:07:0", 1)]).toDF("date", "val")
display(df)

# Give me the start and end of time window in 10-minute Group
winDF = df.groupBy(window("date", "10 minutes")).agg(sum("val").alias("sum"))
display(winDF.select(winDF.window.start.cast("string").alias("start"),
         winDF.window.end.cast("string").alias("end"), "sum"))
