...
df1 = df.select(df.device, df.traffic_source, df.user_id).where(df.event_name == "warranty").withColumn("new", df.user_id)
...
# What is this object?  See Output
...
eventCountsDF = df.groupBy("device").count()
...
display(avgStatePurchasesDF).explain(mode = "CODEGEN")
...
# 'agg' allows you to do multiple aggregations (aka: avg, sum, min, max)
# Advantage is only have to 'groupBy' once
