...
# MAGIC %sql
# MAGIC -- Fix SSN so they all have 2 hypens
# MAGIC DROP TABLE IF EXISTS FixSSN;
# MAGIC 
# MAGIC CREATE TABLE FixSSN AS
# MAGIC (SELECT firstName, middleName, lastName, gender, birthDate, salary
# MAGIC        ,lower(firstName) as firstNameLow, lower(middleName) as middleNameLow, lower(lastName) as lastNameLow,
# MAGIC        CASE
# MAGIC          WHEN length(ssn) = 9 THEN SUBSTRING(ssn, 1, 3) || '-' || SUBSTRING(ssn, 4,2) || '-' || SUBSTRING(ssn, 6,4) 
# MAGIC          ELSE  ssn
# MAGIC        END as ssn
# MAGIC  FROM people_view
# MAGIC  );

# COMMAND ----------

# MAGIC %sql
# MAGIC -- Confirm Row 74 is now fixed
# MAGIC SELECT * FROM FixSSN

# COMMAND ----------

# MAGIC %sql
# MAGIC -- Keep Original case for Names, but squeeze out Dups.  Should have 100,000 rows
# MAGIC SELECT firstName, middleName, lastName, ssn FROM (
# MAGIC        SELECT firstName, middleName, lastName, ssn, firstNameLow, middleNameLow, lastNameLow
# MAGIC       ,row_number() OVER (partition by firstNameLow, middleNameLow, lastNameLow order by firstNameLow DESC) as rank 
# MAGIC FROM FixSSN) as dt 
# MAGIC WHERE dt.rank = 1

# COMMAND ----------

# MAGIC %sql
# MAGIC -- Confirm have 100,000 rows
# MAGIC SELECT count(*) FROM
# MAGIC (SELECT firstName, middleName, lastName, ssn FROM (
# MAGIC         SELECT firstName, middleName, lastName, ssn, firstNameLow, middleNameLow, lastNameLow
# MAGIC        ,row_number() OVER (partition by firstNameLow, middleNameLow, lastNameLow order by firstNameLow DESC) as rank 
# MAGIC FROM FixSSN)
# MAGIC as dt WHERE dt.rank = 1)
...
dbutils.fs.ls(destFile)

# COMMAND ----------

# Write to Parquet using 8 Partitions and Count

dedupedDF = spark.sql("""SELECT firstName, middleName, lastName, ssn FROM (
       SELECT firstName, middleName, lastName, ssn, firstNameLow, middleNameLow, lastNameLow
      ,row_number() OVER (partition by firstNameLow, middleNameLow, lastNameLow order by firstNameLow DESC) as rank 
FROM FixSSN)as dt 
WHERE dt.rank = 1""")

destFile = workingDir + "/people.parquet"

(dedupedDF.repartition(8).write
          .mode("overwrite")
          .format("parquet")
          .save(destFile)
)
